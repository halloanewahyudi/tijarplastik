...
<template>
    <div class="container">
      <ul class="flex flex-wrap justify-center items-center gap-3  pb-10">
          <button  @click="product.category = '' " class="py-2 px-4 rounded-lg bg-borderlight" > Semua </button>
        ...
        <button class="flex items-center gap-3" @click="product.prevPage" :disabled="product.currentPage === 1"> <IconPrevVue/> Prev </button>
        <button class="flex items-center gap-3" @click="product.nextPage" :disabled="product.currentPage === totalPages"> Next <IconNextVue/> </button>
      </div>
    </div>
</template>